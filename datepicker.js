...
function getDateString (dt) {
    return dt.getDate() + '-' + monthsAbbrev[dt.getMonth()] + '-' + dt.getFullYear();
}
...
function chooseDate (e) {
    ...
    var idOfTextbox = div.getAttribute('data-datepickertextbox'); // Get the textbox id which was saved in the div
    var textbox = document.getElementById(idOfTextbox); // Find the textbox now
    if (targ.value === '<' || targ.value === '>' || targ.value === '<<' || targ.value === '>>') { // Do they want to change the month?
        createCalendar(div, new Date(targ.getAttribute('data-date')));
        return;
    }
    textbox.value = targ.getAttribute('data-date'); // Set the selected date
...
function parseMyDate (d) {
    ...
    var m = monthsAbbrev.indexOf(a[1]);
...
function createCalendar (div, month) {
    ...
    lastYearBn.value = '<<';
    lastYearBn.addEventListener('click', chooseDate);
    lastYearBn.setAttribute('data-date', new Date(month.getFullYear(), month.getMonth() - 12, 1, 0, 0, 0, 0).toString());
    ...
    lastMonthBn.value = '<';
    lastMonthBn.addEventListener('click', chooseDate);
    lastMonthBn.setAttribute('data-date', new Date(month.getFullYear(), month.getMonth() - 1, 1, 0, 0, 0, 0).toString());
    ...
    nextMonthBn.value = '>';
    nextMonthBn.addEventListener('click', chooseDate);
    nextMonthBn.setAttribute('data-date', new Date(month.getFullYear(), month.getMonth() + 1, 1, 0, 0, 0, 0).toString());
    ...
    nextYearBn.value = '>>';
    nextYearBn.addEventListener('click', chooseDate);
    nextYearBn.setAttribute('data-date', new Date(month.getFullYear(), month.getMonth() + 12, 1, 0, 0, 0, 0).toString());

    var daysRow = tbl.insertRow(-1);
    weekDaysAbbrev.forEach(function (weekDayAbbrev) {
        daysRow.insertCell(-1).textContent = weekDayAbbrev;
    });
    ...
    while (1) { // Loop for each week
        ...
        for (var i = 0; i < 7; i++) { // Loop each day of this week
            ...
            inp.addEventListener('click', chooseDate);
            inp.setAttribute('data-date', getDateString(date));
...
function showDatePicker (idOfTextbox) {
    ...
    for (var i = 0; i < x.length; i++) {
        if (x[i].className === 'datepickerdropdown') {
    ...
    div.setAttribute('data-datepickertextbox', idOfTextbox); // Remember the textbox id in the div
...
window.addEventListener('DOMContentLoaded', function () {
    var datepickers = Array.from(document.getElementsByClassName('datepicker'));
